...
const elements = {
  ...
  refreshSystemButton: document.querySelector("#refreshSystemButton"),
...
function setSystemFieldsLocked(isLocked) {
  [elements.sscSettlement, elements.systemSale].forEach((input) => {
...
function apiErrorMessage(response, payload) {
  return payload.error || `System sale lookup failed with status ${response.status}.`;
}

async function fetchSystemSalePayload(date) {
  const query = `store=${encodeURIComponent(activeStore.id)}&date=${encodeURIComponent(date)}`;
  const endpoint = `/api/system-sale?${query}`;
  ...
  try {
    ...
    throw new Error(`Could not fetch system sale: ${error.message}`);
  }
}

async function autoFillSystemValues(date = elements.entryDate.value, options = {}) {
  if (!activeStore || !date) return;

  const requestId = ++systemRefreshRequestId;
  const isManual = Boolean(options.manual);
  elements.dbStatus.textContent = "Loading system sale...";
  if (elements.refreshSystemButton) elements.refreshSystemButton.disabled = true;

  try {
    const payload = await fetchSystemSalePayload(date);
    if (requestId !== systemRefreshRequestId || date !== elements.entryDate.value) return;

    elements.systemSale.value = payload.todaysale ?? "";
    elements.sscSettlement.value = payload.sambridhi ?? "";
    setSystemFieldsLocked(true);
    calculate();
    elements.dbStatus.textContent = "System sale loaded";
    if (isManual) {
      elements.saveMessage.textContent = `Refreshed SALE in system and SSC Payments for ${date}.`;
    }
  } catch (error) {
    setSystemFieldsLocked(false);
    elements.dbStatus.textContent = "System sale not loaded";
    elements.saveMessage.textContent = `Could not auto-load system values: ${error.message}`;
    console.error(error);
  } finally {
    if (elements.refreshSystemButton) elements.refreshSystemButton.disabled = false;
  }
}

function startSystemRefreshTimer() {
  stopSystemRefreshTimer();
  systemRefreshTimer = window.setInterval(() => {
    if (!activeStore || elements.appShell.classList.contains("hidden")) return;
    if (currentEntryDate) return;
    autoFillSystemValues(elements.entryDate.value);
  }, 30000);
}

function stopSystemRefreshTimer() {
...
function attachEvents() {
  ...
  elements.entryForm.addEventListener("input", () => calculate());
  elements.entryForm.addEventListener("change", () => calculate());
  ...
  elements.entryDate.addEventListener("change", () => autoFillSystemValues(elements.entryDate.value));
  ...
  elements.saveButton.addEventListener("click", async () => {
    ...
    try {
      ...
      elements.saveMessage.textContent = "Saved. Ready for the next entry.";
    } catch (error) {
  ...
  elements.refreshSystemButton.addEventListener("click", () => autoFillSystemValues(elements.entryDate.value, { manual: true }));
